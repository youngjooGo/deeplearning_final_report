...
\section{Motivation}
\section{Method}
We implemented MCTS for Da Vinci code in two versions. First, Da vinci code is implemented for CPU by using openMP. Second, it is implemented for GPU by using CUDA. Both versions are implemented in parallel. Following section, we describe the detail altogrithm for each version. In addition, we explain the detail algorithm design which is modified unlike the original version due to compuation overhead.
